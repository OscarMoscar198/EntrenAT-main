...
interface ShoulderModalProps {
  visible: boolean;
  onClose: () => void;
}

const ShoulderModal: React.FC<ShoulderModalProps> = ({ visible, onClose }) => {
  const exercises = [
    "Military Press",
    "Lateral Raises",
    "Front Raises",
  ...
  const handleSave = async () => {
    try {
      const userId = 2;
      const exercisesId = 5; // Actualizar con el ID correcto
    ...
    >
      <View style={styles.modalContainer}>
        <View style={styles.modalContent}>
          ...
          <Text style={styles.modalHeader}>Ejercicios para Hombros</Text>
